...
Storage.prototype._keys = function() {
  ...
  if (args.length === 0 || args.length === args.functions().length) {
    return [];
  }
  ...
  var bulk_operation = {
    ...
    next: function(key, error, result, response) {
      ...
      if (Object.keys(res).length >= keys.length) {
        ...
        var key_parts = keys.length ? keys[0].split('/') : [0];
        var is_wildcard = (key_parts.length > 1 && key_parts[1] === '*' || key_parts[1] === '' || !key_parts[1]);
...
Storage.prototype._get = function(args, execute) {
  var self = this, args_length = 3;

  args = fun(args);

  if (args.strings().length === 0 && args.arrays().length === 0) {
    args_length = 2
  }

  self._command('get', args, args_length, function(keys, options, callback) {
    var args = fun(arguments);

    keys = args.strings()[0] || args.arrays()[0] || [];
    options = args.objects()[0]
    callback = args.functions()[0];
...
Storage.prototype._del = function(args, execute) {
  ...
  self._command('del', args, args_length, function(keys, options, callback) {
    var _args = fun(arguments);

    keys = _args.strings()[0] || _args.arrays()[0] || [];
...
Storage.prototype._exists = function(args, execute) {
  ...
  self._command('exists', args, args_length, function(keys, options, callback) {
    var args = fun(arguments);
...
// #_count ()
Storage.prototype._count = function(args, execute) {
  var self = this, args_length = 3;
  ...
  self._command('count', args, args_length, function(keys, options, callback) {
    var args = fun(arguments);
...
// #_clear ()
Storage.prototype._clear = function(args, execute) {
  var self = this;

  self._command('clear', args, 3, function(keys, options, callback) {
    var args = fun(arguments);
...
// #count (key, [options], callback)
// #count (keys, [options], callback)
Storage.prototype.count = function() {
  throw new Error("Not implemented");

  // var self = this;
  //
  // self._count(arguments, function(keys, options, done) {
  //   // noop
  // });
};

// #clear (key, [options], callback)
// #clear (keys, [options], callback)
Storage.prototype.clear = function() {
  throw new Error("Not implemented");

  // var self = this;
  //
  // self._clear(arguments, function(keys, options, done) {
  //   // noop
  // });
};
