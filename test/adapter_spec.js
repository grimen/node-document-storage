    ...
    Spec[name + '.prototype'] = {
          ...
          '<NEW_KEY>': {
            "(<STRING_KEY>, <VALUE>)  =>  [true]": function(done) {
              storage.set('set/new-one-foo_1-a', {foo: 'bar_1'}, function(storage_err, storage_response) {
                spec.client.get(db, 'set', 'new-one-foo_1-a', function(client_err, client_response) {
          ...
          '[<NEW_KEY>]': {
            "([<STRING_KEY>], [<VALUE>])  =>  [true]": function(done) {
              storage.set(['set/new-one-foo_1-b'], [{foo: 'bar_1'}], function(storage_err, storage_response) {
                spec.client.get(db, 'set', 'new-one-foo_1-b', function(client_err, client_response) {
                  client_response = Array.create(client_response).map(function(doc) { return doc && pack(Object.reject(typeof doc === 'string' ? unpack(doc) : doc, meta_fields)); });
                  assert.deepEqual ( storage_response, [true] );
                  assert.deepEqual ( client_response[0], pack({foo: 'bar_1'}) );
                  done();
          ...
          '[<NEW_KEY>, <NEW_KEY]': {
            "([<STRING_KEY_1>, <STRING_KEY_2>], [<VALUE_1>, <VALUE_2>])  =>  [true, true]": function(done) {
              storage.set(['set/new-many-foo_1-c', 'set/new-many-foo_2-c'], [{foo: 'bar_1'}, {foo: 'bar_2'}], function(storage_err, storage_response) {
                spec.client.get(db, 'set', 'new-many-foo_1-c', function(client_err_1, client_response_1) {
                  spec.client.get(db, 'set', 'new-many-foo_2-c', function(client_err_2, client_response_2) {
        ...
        'one': {
          '<NEW_KEY>': {
            "(<NEW_KEY>)  =>  [false]": function(done) {
              storage.del('del/new-one-foo_1-a', function(storage_err, storage_response) {
                spec.client.get(db, 'del', 'new-one-foo_1-a', function(client_err, client_response) {
          ...
          '<EXISTING_KEY>': {
            "(<EXISTING_KEY>)  =>  [true]": function(done) {
              spec.client.set(db, 'del', 'existing-one-foo_1-b', pack({foo: 'bar_1'}), function() {
                storage.del('del/existing-one-foo_1-b', function(storage_err, storage_response) {
                  spec.client.get(db, 'del', 'existing-one-foo_1-b', function(client_err, client_response) {
                    assert.deepEqual ( storage_response, [true] );
                    assert.deepEqual ( client_response, null );
                    done();
                  });
          ...
          '[<NEW_KEY>]': {
            "([<NEW_KEY>])  =>  [false]": function(done) {
              storage.del(['del/new-one-foo_1-c'], function(storage_err, storage_response) {
                spec.client.get(db, 'del', 'new-one-foo_1-c', function(client_err, client_response) {
                  assert.deepEqual ( storage_response, [false] );
                  assert.deepEqual ( client_response, null );
                  done();
                });
          ...
          '[<EXISTING_KEY>]': {
            "([<EXISTING_KEY>])  =>  [true]": function(done) {
              spec.client.set(db, 'del', 'existing-one-foo_1-d', pack({foo: 'bar_1'}), function() {
                storage.del(['del/existing-one-foo_1-d'], function(storage_err, storage_response) {
                  spec.client.get(db, 'del', 'existing-one-foo_1-d', function(client_err, client_response) {
                    assert.deepEqual ( storage_response, [true] );
                    assert.deepEqual ( client_response, null );
                    done();
                  });
        ...
        'many': {
          '[<NEW_KEY>, <NEW_KEY>]': {
            "([<NEW_KEY>, <NEW_KEY>])  =>  [false, false]": function(done) {
              storage.del(['del/new-many-foo_1-a', 'del/new-many-foo_2-a'], function(storage_err, storage_response) {
                spec.client.get(db, 'del', 'new-many-foo_1-a', function(client_err_1, client_response_1) {
                  spec.client.get(db, 'del', 'new-many-foo_2-a', function(client_err_2, client_response_2) {
          ...
          '[<NEW_KEY>, <EXISTING_KEY>]': {
            "([<NEW_KEY>, <EXISTING_KEY>])  =>  [false, true]": function(done) {
              spec.client.set(db, 'del', 'existing-many-foo_1-b', pack({foo: 'bar_1'}), function() {
                storage.del(['del/new-many-foo_1-b', 'del/existing-many-foo_1-b'], function(storage_err, storage_response) {
                  spec.client.get(db, 'del', 'new-many-foo_1-b', function(client_err_1, client_response_1) {
                    spec.client.get(db, 'del', 'existing-many-foo_1-b', function(client_err_2, client_response_2) {
          ...
          '[<EXISTING_KEY>, <NEW_KEY>]': {
            "([<EXISTING_KEY>, <NEW_KEY>])  =>  [true, false]": function(done) {
              spec.client.set(db, 'del', 'existing-many-foo_1-c', pack({foo: 'bar_1'}), function() {
                storage.del(['del/existing-many-foo_1-c', 'del/new-many-foo_1-c'], function(storage_err, storage_response) {
                  spec.client.get(db, 'del', 'existing-many-foo_1-c', function(client_err_1, client_response_1) {
                    spec.client.get(db, 'del', 'new-many-foo_1-c', function(client_err_2, client_response_2) {
          ...
          '[<EXISTING_KEY>, <EXISTING_KEY>]': {
            "([<EXISTING_KEY>, <EXISTING_KEY>])  =>  [true, true]": function(done) {
              spec.client.set(db, 'del', 'existing-many-foo_1-d', pack({foo: 'bar_1'}), function() {
                spec.client.set(db, 'del', 'existing-many-foo_2-d', pack({foo: 'bar_2'}), function() {
                  storage.del(['del/existing-many-foo_1-d', 'del/existing-many-foo_2-d'], function(storage_err, storage_response) {
                    spec.client.get(db, 'del', 'existing-many-foo_1-d', function(client_err_1, client_response_1) {
                      spec.client.get(db, 'del', 'existing-many-foo_2-d', function(client_err_2, client_response_2) {
                        assert.deepEqual ( storage_response, [true, true] );
                        assert.deepEqual ( [client_response_1, client_response_2], [null, null] );
                        done();
                      });
      ...
      '#exists': {
        'one': {
          ...
          '<EXISTING_KEY>': {
            "(<EXISTING_KEY>)  =>  [true]": function(done) {
              spec.client.set(db, 'exists', 'existing-one-foo_1-a', pack({foo: 'bar_1'}), function() {
          ...
          '[<EXISTING_KEY>]': {
            "([<EXISTING_KEY>])  =>  [true]": function(done) {
              spec.client.set(db, 'exists', 'existing-one-foo_1-c', pack({foo: 'bar_1'}), function() {
        ...
        'many': {
          ...
          '[<NEW_KEY>, <EXISTING_KEY>]': {
            "([<NEW_KEY>, <EXISTING_KEY>])  =>  [false, true]": function(done) {
              spec.client.set(db, 'exists', 'existing-many-foo_1-b', pack({foo: 'bar_1'}), function() {
          ...
          '[<EXISTING_KEY>, <NEW_KEY>]': {
            "([<EXISTING_KEY>, <NEW_KEY>])  =>  [true, false]": function(done) {
              spec.client.set(db, 'exists', 'existing-many-foo_1-c', pack({foo: 'bar_1'}), function() {
          ...
          '[<EXISTING_KEY>, <EXISTING_KEY>]': {
            "([<EXISTING_KEY>, <EXISTING_KEY>])  =>  [true, true]": function(done) {
              spec.client.set(db, 'exists', 'existing-many-foo_1-d', pack({foo: 'bar_1'}), function() {
                spec.client.set(db, 'exists', 'existing-many-foo_2-d', pack({foo: 'bar_2'}), function() {
